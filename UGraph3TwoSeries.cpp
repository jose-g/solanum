...
#pragma link "TeeURL"
#pragma resource "*.dfm"
TfrmGraph3TwoSeries *frmGraph3TwoSeries;
//---------------------------------------------------------------------------
__fastcall TfrmGraph3TwoSeries::TfrmGraph3TwoSeries(TComponent* Owner)
        : TForm(Owner)
{
  filename=NULL;
...
//---------------------------------------------------------------------------
void TfrmGraph3TwoSeries::EnterInformation(char* _cond)
{
strcpy(filename,_cond);
}
//---------------------------------------------------------------------------
void __fastcall TfrmGraph3TwoSeries::SpeedButton4Click(TObject *Sender)
{
Close();
}
//---------------------------------------------------------------------------
void __fastcall TfrmGraph3TwoSeries::chbCCClick(TObject *Sender)
{
  tch1->Series[0]->Marks->Visible=chbCC->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmGraph3TwoSeries::chbCCwClick(TObject *Sender)
{
  tch1->Series[1]->Marks->Visible=chbCCw->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmGraph3TwoSeries::FormShow(TObject *Sender)
{
  tc1->Panel=tch1;
  FILE * pFile;
...
void __fastcall TfrmGraph3TwoSeries::chbCCnClick(TObject *Sender)
{
  tch1->Series[2]->Marks->Visible=chbCCn->Checked;
}
//---------------------------------------------------------------------------

void __fastcall TfrmGraph3TwoSeries::chbCCfClick(TObject *Sender)
{
...
void __fastcall TfrmGraph3TwoSeries::sbSaveClick(TObject *Sender)
{
  ...
  if(sdSave->Execute())
  {
    ...
    if(sdSave->FilterIndex==1)
    {
      ...
      fprintf(pFileOutput,"%s %s %s %s %s\n","Day","   Canopy cover potential","   Canopy cover by water","   Canopy cover by nitrogen","   Canopy cover by frost");
...
void __fastcall TfrmGraph3TwoSeries::sdSaveSelectionChange(TObject *Sender)
{
if(sdSave->FilterIndex==1)
{
